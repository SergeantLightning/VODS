...
function run() {
	...
	*/
		...
		if (RAM[target] == 255) {
			...
			temp += 1;
			RAM[target] = temp.toString(16).padStart(2, "0");
		}
	} else if (byte == "f6") {
		PC += 1;
		var target = parseInt(RAM[PC], 16);
	...
	*/
		...
		A = "0" + A.slice(0, 7);
		A = Number(A.toString(10));
		updateFlagsByReg("A");
	} else if (byte == "46") { // ZP
		PC += 1;
		var target = parseInt(RAM[PC], 16);
		F[0] = Number(RAM[target].toString(2).padStart(8, "0").charAt(7));
		RAM[target] = RAM[target].toString(2).padStart(8, "0");
		RAM[target] = "0" + RAM[target].slice(0, 7);
		RAM[target] = RAM[target].toString(16);
		updateFlagsByRAM(target);
	} else if (byte == "56") {
		PC += 1;
		var target = parseInt(RAM[PC], 16);
